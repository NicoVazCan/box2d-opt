...
struct Vec2
{
	...
	constexpr void operator *= (float a)
	{
		x *= a; y *= a;
	}
...
struct Mat22
{
	...
	constexpr Mat22(const Vec2& col1, const Vec2& col2) : col1(col1), col2(col2) {}

	constexpr Mat22 Transpose() const
	{
...
static constexpr Vec2 Cross(const Vec2& a, float s)
{
	return Vec2(s * a.y, -s * a.x);
}

static constexpr Vec2 Cross(float s, const Vec2& a)
{
	return Vec2(-s * a.y, s * a.x);
}

static constexpr Vec2 operator * (const Mat22& A, const Vec2& v)
{
	return Vec2(A.col1.x * v.x + A.col2.x * v.y, A.col1.y * v.x + A.col2.y * v.y);
}

static constexpr Vec2 operator + (const Vec2& a, const Vec2& b)
{
...
static constexpr Vec2 operator * (float s, const Vec2& v)
{
	return Vec2(s * v.x, s * v.y);
}

static constexpr Mat22 operator + (const Mat22& A, const Mat22& B)
{
...
static constexpr Vec2 Abs(const Vec2& a)
{
	return Vec2(fabsf(a.x), fabsf(a.y));
}

static constexpr Mat22 Abs(const Mat22& A)
{
...
static constexpr float Clamp(float a, float low, float high)
{
