...
static inline float randf() {
  return float(my_random() & 0xffff) / float(0xffff);
}

static inline float randf(float val) {
  return val * float(my_random() & 0xffff) / float(0xffff);
}
...
int main(int argc, char **args) {
  ...
  std::array<bvh::index_t, 4> indices;
  std::array<bounce_t, 4> bounce;
